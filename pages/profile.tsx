...
// Quick links data
const QUICK_LINKS = [
  { key: "liked-songs", label: "آهنگ‌های لایک‌شده", icon: ICONS.heart },
  {
    key: "liked-playlists",
    label: "پلی‌لیست‌های لایک‌شده",
    icon: ICONS.playlist,
  },
  { key: "followed-artists", label: "هنرمندان دنبال‌شده", icon: ICONS.users },
  { key: "my-playlists", label: "پلی‌لیست‌های من", icon: ICONS.folder },
];
        ...
        <div className="flex items-center px-6 pt-6 justify-between">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-white/5 rounded-xl border border-white/10">
              <Icon d={ICONS.music} className="w-6 h-6 text-emerald-400" />
            </div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 hide-scrollbar">
                  {Array.from({ length: 9 }).map((_, idx) => (
                    <SongCard
                      ...
                      title={`ترانه ${idx + 1}`}
                      artist={`هنرمند ${idx + 1}`}
                      playedAt={idx % 2 === 0 ? "دیروز" : "2 روز پیش"}
                      playCount={(idx + 1) * 142}
                      onClick={() => console.log("play", idx)}
                    />
                  ))}
