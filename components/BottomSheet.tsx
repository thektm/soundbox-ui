...
const BottomSheet = forwardRef((props: BottomSheetProps, ref: React.Ref<BottomSheetHandle>) => {
  const {
  ...
  const isOpen = typeof controlledIsOpen === 'boolean' ? controlledIsOpen : localOpen;
  ...
  useImperativeHandle(ref, () => ({
    open: () => { if (typeof controlledIsOpen !== 'boolean') setLocalOpen(true); },
    close: () => { if (typeof controlledIsOpen !== 'boolean') setLocalOpen(false); snapTo(-1); },
    toggle: () => { if (typeof controlledIsOpen !== 'boolean') setLocalOpen(v => !v); },
    snapTo: (index: number) => snapTo(index),
    isOpen: () => (typeof controlledIsOpen === 'boolean' ? controlledIsOpen : localOpen),
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className={`bg-gray-900 border-t border-white/10 shadow-2xl flex flex-col overflow-hidden transition-[border-radius] duration-200 ${isAtTop ? 'rounded-t-none' : 'rounded-t-3xl'}`}
              style={{
...
}) as React.ForwardRefExoticComponent<React.PropsWithoutRef<BottomSheetProps> & React.RefAttributes<BottomSheetHandle>>;
...
export function useBottomSheet() {
  ...
  const snapToRef = useCallback((i: number) => { ref.current?.snapTo(i); }, []);
